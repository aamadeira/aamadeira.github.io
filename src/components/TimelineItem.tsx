...
export default function TimelineItem({ title, subtitle, date, isLast = false, index = 0, children }: TimelineItemProps) {
    return (
        <motion.div
        ...
        >
            ...
            <div className={cn("pb-8", isLast ? "pb-0" : "")}>
                ...
                <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.5, delay: index * 0.2 + 0.4 }} viewport={{ once: true, margin: "-50px" }}>
                    {children}
                </motion.div>
            </div>
        </motion.div>
